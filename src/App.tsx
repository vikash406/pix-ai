...
export default function App() {
  ...
  const [footerPage, setFooterPage] = useState<'none' | 'about' | 'contact' | 'privacy'>('none');
  ...
  useEffect(() => {
    ...
    const lowEnd = deviceMemory <= 4 || cpuCores <= 4;
  ...
  const handleEnhance = async () => {
    ...
    try {
      ...
      const fgBlob = await removeBackground(blob, {
        progress: (step: string, curr: number, total: number) => {
          const p = Math.round((curr / total) * 20) + 15; // 15-35%
      ...
      // Enhance foreground: choose a lighter resolution for low-end devices
      const canvas = document.createElement('canvas');
      const maxWidth = isLowEndDevice ? 1280 : 3840;
      const targetWidth = Math.min(maxWidth, Math.max(640, fgImg.width));
      const ratio = targetWidth / fgImg.width;
      canvas.width = targetWidth;
      canvas.height = Math.round(fgImg.height * ratio);
      ...
      if (!isLowEndDevice) {
        // Sharpening convolution for standard devices only
      ...
      setProgress(isLowEndDevice ? 70 : 75);

      // Post-processing: increased brightness, contrast, saturation
      const tempCanvas = document.createElement('canvas');
      tempCanvas.width = canvas.width;
      tempCanvas.height = canvas.height;
      const tempCtx = tempCanvas.getContext('2d');
      if (tempCtx) {
        tempCtx.filter = 'contrast(1.15) saturate(1.15) brightness(1.2)'; // Clear and Increased brightness
      ...
      if (bgCtx) {
        ...
        blurTemp.height = bgCanvas.height;
        const blurCtx = blurTemp.getContext('2d');
        if (blurCtx) {
          blurCtx.filter = `blur(${blurAmount}px)`;
          blurCtx.drawImage(bgCanvas, 0, 0);
          bgCanvas.width = blurTemp.width;
          bgCanvas.height = blurTemp.height;
          bgCtx.drawImage(blurTemp, 0, 0);
        }
      }
      setProgress(95);

      // Composite: blurred background + enhanced foreground
      const finalCanvas = document.createElement('canvas');
      finalCanvas.width = canvas.width;
      finalCanvas.height = canvas.height;
      const finalCtx = finalCanvas.getContext('2d');
      if (finalCtx) {
        finalCtx.drawImage(bgCanvas, 0, 0); // Blurred background
  ...
  const openFooterPage = (page: 'about' | 'contact' | 'privacy') => {
    setFooterPage(page);
    setActiveTab('none');
  };

  const footerPages: Record<'about' | 'contact' | 'privacy', { title: string; description: string[] }> = {
    about: {
      title: 'About Pix AI',
      description: [
        'Welcome to Pix AI — a smart and simple AI-powered image editing platform.',
        'Our goal is to make image enhancement fast, easy, and accessible for everyone. With Pix AI, users can enhance image quality, upscale resolution, and remove backgrounds using advanced AI technology.',
        'We focus on delivering high-quality results with a smooth and user-friendly experience. Whether you are a content creator, student, or business owner, Pix AI helps you create better visuals instantly.',
        'Our mission is to simplify AI tools and make them available to everyone.',
        'Thank you for using Pix AI.',
      ],
    },
    contact: {
      title: 'Contact Us',
      description: [
        'Have feedback or need support? Reach out to us by email at hello@pixai.example.com.',
        'This app is built to keep image editing straightforward and accessible on every device.',
        'We welcome suggestions to make mobile and low-end performance even better.',
        'Your feedback helps us improve Pix AI and provide a better experience.',
    ...
    privacy: {
      ...
      description: [
        'At Pix AI, we respect your privacy and are committed to protecting your data.',
        'We do not collect personal data unless you contact us directly.',
        'All images uploaded on Pix AI are processed securely. We do not store your images permanently on our servers.',
        'We may use cookies to improve user experience and analyze website traffic.',
        'We may use third-party services like ads networks which may use cookies and tracking technologies.',
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white font-sans selection:bg-white selection:text-black">
      ...
      <main className="relative z-10 max-w-7xl mx-auto p-6 md:p-12">
        <AnimatePresence mode="wait">
          {footerPage !== 'none' ? (
            ...
            >
              <button
                onClick={() => setFooterPage('none')}
                className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-zinc-800 bg-zinc-950/80 text-zinc-300 hover:bg-zinc-900 transition"
              >
                <ArrowRight className="w-4 h-4 rotate-180" />
                Back to App
              </button>

              <div className="mt-8 rounded-[32px] border border-zinc-800 bg-zinc-900/80 p-8 shadow-xl shadow-black/20">
                <h2 className="text-3xl font-semibold tracking-tight text-white">
                  {footerPages[footerPage].title}
                </h2>
                <div className="mt-5 space-y-4 text-zinc-400 text-sm leading-relaxed">
                  {footerPages[footerPage].description.map((line, index) => (
                    <p key={index}>{line}</p>
                  ))}
                </div>
              </div>
            ...
            >
              ...
              <p className="text-zinc-500 mb-10 text-sm md:text-base leading-relaxed text-balance">
                Upscale images to 4K, sharpen details, boost brightness, blur backgrounds, and compress for fast sharing. 
            ...
            >
              <div className="lg:col-span-8 flex flex-col gap-6">
                <div className="flex items-center justify-between">
                  ...
                  {isLowEndDevice && (
                    <div className="flex items-center gap-2 px-3 py-1 bg-sky-500/10 text-sky-300 rounded-full border border-sky-500/20">
                      <span className="text-[10px] uppercase font-semibold tracking-tighter">Low-end mode enabled</span>
                    </div>
                  )}
                </div>
                ...
                <div className="p-4 rounded-xl bg-zinc-900/50 border border-zinc-800 flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <p className="text-xs text-zinc-400 leading-relaxed uppercase tracking-tight">
                      Use the slider to compare results. Enhancement sharpens and brightens the foreground, blurs the background, and compresses for optimal file size. On low-end devices we use lighter processing to keep the app responsive.
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  >
                    <div className="flex items-center gap-4 text-left">
                      ...
                      <div>
                        <p className="font-bold text-sm uppercase tracking-tight">Sharpen & Enhance</p>
                        <p className={cn("text-[10px] uppercase tracking-widest", activeTab === 'enhance' ? "text-black/60" : "text-zinc-500")}>Clear, Bright, Blur BG</p>
                      </div>
                    </div>
      ...
      <footer className="relative z-10 max-w-7xl mx-auto px-6 py-6 md:px-12 md:py-8 border-t border-zinc-900 text-zinc-400 text-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-wrap items-center gap-3 text-xs uppercase tracking-[0.2em] text-zinc-500">
            <button onClick={() => openFooterPage('about')} className="rounded-full px-3 py-2 hover:bg-zinc-900 transition">About</button>
            <button onClick={() => openFooterPage('contact')} className="rounded-full px-3 py-2 hover:bg-zinc-900 transition">Contact</button>
            <button onClick={() => openFooterPage('privacy')} className="rounded-full px-3 py-2 hover:bg-zinc-900 transition">Privacy Policy</button>
          </div>
          <p className="text-[11px] text-zinc-500">© 2026 Pix AI. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
